...
- **Керування сесіями:** Дізнайтеся, як ефективно використовувати сесії OpenCode — [читайте посібник](docs/sessions.uk.md). ⚠️ **Важливо:** Без розуміння того, як працюють сесії та контекст, ви не зможете ефективно працювати з жодним ШІ-асистентом. Це фундаментальна навичка для досягнення результату.
- **Пресет Developer:** Автоматичне встановлення gitui, спеціалізовані робочі середовища WezTerm та гарячі клавіші для AI-кодингу з Git. [Детальніше](docs/developer-tools.uk.md).
- **Спільний термінал:** Виконуйте команди у спільному вікні терміналу прямо з OpenCode. [Детальніше](docs/shared-terminal.uk.md).
...
### [Сервери Model Context Protocol (MCP)](docs/mcp/mcp.uk.md)

MCP-сервери розширюють можливості ШІ, надаючи доступ до локальних систем та інструментів. [Детальніше про доступні MCP-сервери](docs/mcp/mcp.uk.md).
...
| `./OpenCodeWizard.sh --silent [preset]` | Автоматичне встановлення з опціональним пресетом (developer\|standard\|minimal) |
...
Майстер включає три пресети, які контролюють кількість встановлених плагінів та MCP-серверів. Вибір пропонується під час налаштування, або використовується **Standard** (за замовчуванням).

| Пресет | Плагіни | MCP-сервери | Для кого |
| :--- | :--- | :--- | :--- |
| **🔥 Developer** | Всі плагіни + [dev tools](docs/developer-tools.uk.md) | Всі MCP | Повноцінне AI-середовище |
| **✨ Standard** | Базові плагіни | Основні MCP | Збалансовано — загальне використання |
| **❄️ Minimal** | Мінімальне налаштування | Базові MCP | Найшвидше — тільки необхідне |
...
- **Гарячі клавіші:**
  - `CTRL + SHIFT + O`: Розділити екран: gitui ліворуч (40%), поточна панель праворуч — запустіть OpenCode вручну у правій панелі.
...
> 🛠️ **Пресет Developer** додає `CTRL+SHIFT+G` (таб gitui), `CTRL+SHIFT+O` (split gitui) та `CTRL+SHIFT+ALT+G` (workspace). [Повний опис →](docs/developer-tools.uk.md)
