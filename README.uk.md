...
- **Контекстне меню Windows:** Опціонально додає "Open in OpenCode" до контекстного меню папок у Провіднику — клацніть правою кнопкою на будь-якій папці та запустіть OpenCode одразу в ній.
...
> **💡 Порада про контекстне меню:** Після налаштування ви можете клацнути правою кнопкою на будь-якій папці у Провіднику та вибрати **"Open in OpenCode"**, щоб запустити OpenCode в цій папці. На **Windows 11** цей пункт з'являється в **класичному контекстному меню** — натисніть **`Shift + F10`** або виберіть **"Показати більше параметрів"** (Show more options).
